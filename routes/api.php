...
|--------------------------------------------------------------------------
|
| Se restringe a los dominios configurados en CENTRAL_DOMAINS para NO chocar
...
$canonicalCentralDomain = CentralDomains::canonicalDomain($centralDomains, (string) config('app.url'));

foreach ($centralDomains as $centralDomain) {
    $storageRouteName = $centralDomain === $canonicalCentralDomain
        ? 'storage.file'
        : 'storage.file.alternate.'.substr(hash('sha256', $centralDomain), 0, 12);

    Route::domain($centralDomain)->group(function () use ($storageRouteName) {
        Route::post('/login', [PlatformAuthController::class, 'login'])->middleware('throttle:login');
        ...
        /*
         ...
         |      'X-Tenant' (= clave interna del tenant) y cambia la conexion a la BD del
        ...
        Route::middleware([
            ...
            ->group(function () {
                // Echo necesita resolver el tenant ANTES de autenticar el token
                // sombra. Se usa una URI distinta de la plataforma para evitar
                // que Laravel haga match primero con /broadcasting/auth central.
